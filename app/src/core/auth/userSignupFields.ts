...
// Hook into password validation during signup
export function validateSignupPassword(password: string, email: string): void {
  const result = validatePasswordStrength(password, [email]);
  
  if (!result.isValid) {
    const errorMessages = [result.feedback.warning, ...result.feedback.suggestions]
      .filter(Boolean)
      .join('. ');
    throw new Error(`Password is too weak. ${errorMessages}`);
  }
}
